...
# Profile allowlists to keep release variants isolated.
DEFAULT_TOOL_PROFILES = {
    'default_profile': 'wreckhunter',
    'profiles': {
        'wreckhunter': list(AVAILABLE_TOOLS.keys()),
        'base': ['thermal', 'optical', 'sar', 'triple_lock'],
        'research': ['thermal', 'optical', 'sar', 'triple_lock', 'swot', 'atl13'],
    },
}
...
class AIDirector:
    ...
    def __init__(self, profile: Optional[str] = None, profile_config_path: Optional[str] = None):
        self.results = []
        self.config = {
            'bbox': None,
            'tools': [],
        ...
        self._profiles_config = self._load_profiles_config(profile_config_path)
        self.active_profile = profile or self._profiles_config.get('default_profile', 'wreckhunter')
        if self.active_profile not in self._profiles_config.get('profiles', {}):
            fallback = self._profiles_config.get('default_profile', 'wreckhunter')
            print(f"⚠ Unknown profile '{self.active_profile}', falling back to '{fallback}'")
            self.active_profile = fallback
        self.config['profile'] = self.active_profile

    def _load_profiles_config(self, profile_config_path: Optional[str]) -> Dict:
        """Load tool profile allowlists from JSON; fallback to built-in defaults."""
        candidate_paths: List[Path] = []
        if profile_config_path:
            candidate_paths.append(Path(profile_config_path))
        candidate_paths.append(Path(__file__).parent / 'tool_profiles.json')
        ...
        for path in candidate_paths:
            try:
                ...
                print(f"⚠ Failed to load profile config from {path}: {exc}")

        return DEFAULT_TOOL_PROFILES

    def _allowed_tools_for_profile(self, profile_name: str) -> List[str]:
        profiles = self._profiles_config.get('profiles', {})
        allowed = profiles.get(profile_name, [])
        if '*' in allowed:
            return list(AVAILABLE_TOOLS.keys())
        return [tool for tool in allowed if tool in AVAILABLE_TOOLS]

    def is_tool_enabled(self, tool_name: str) -> bool:
        return tool_name in self._allowed_tools_for_profile(self.active_profile)

    def get_tools_for_display(self, show_disabled: bool = False) -> Dict[str, Dict]:
        if show_disabled:
            return AVAILABLE_TOOLS
        allowed = set(self._allowed_tools_for_profile(self.active_profile))
        return {tool_id: meta for tool_id, meta in AVAILABLE_TOOLS.items() if tool_id in allowed}
    ...
    def parse_request(self, request: str) -> Dict:
        ...
        # Profile gate: only allow tools enabled for the active release profile.
        tools = [t for t in tools if self.is_tool_enabled(t)]

        # Default to all sensors if none specified
        if not tools:
            defaults = ['thermal', 'optical']
            tools = [t for t in defaults if self.is_tool_enabled(t)]
    ...
    def set_tools(self, tools: List[str]):
        ...
        blocked_tools = [t for t in valid_tools if not self.is_tool_enabled(t)]
        valid_tools = [t for t in valid_tools if self.is_tool_enabled(t)]
        self.config['tools'] = valid_tools
        print(f"✓ Tools selected: {', '.join(valid_tools)}")
        if blocked_tools:
            print(f"⚠ Blocked by profile '{self.active_profile}': {', '.join(blocked_tools)}")
    ...
    def run_tool(self, tool_name: str) -> Dict:
        ...
        if not self.is_tool_enabled(tool_name):
            return {
                'tool': tool_name,
                'success': False,
                'error': f"Tool '{tool_name}' is not enabled in profile '{self.active_profile}'",
            }
    ...
    def execute(self) -> List[Dict]:
        ...
        if not self.config['bbox']:
            self.set_bounding_box('lake_michigan_south')

        print(f"Bounding box: {self.config['bbox']['name']}")
        print(f"Sensitivity: {self.config['sensitivity']}")
        print(f"Tools: {', '.join(self.config['tools']) if self.config['tools'] else 'None'}")

        if not self.config['tools']:
            print("⚠ No executable tools selected for this profile/request")
            self.results = []
            return []
...
def main():
    ...
    parser.add_argument('--profile', type=str, default='wreckhunter', help='Release profile allowlist (e.g., wreckhunter, base, research)')
    parser.add_argument('--profile-config', type=str, help='Path to tool profile JSON override')
    ...
    parser.add_argument('--show-disabled-tools', action='store_true', help='When listing tools, include tools blocked by the active profile')
    ...
    # Initialize director with active profile before any operation.
    director = AIDirector(profile=args.profile, profile_config_path=args.profile_config)
    ...
    if args.list_tools:
        ...
        print("="*120)
        print(f"Active profile: {director.active_profile}")
        print(f"Show disabled: {'yes' if args.show_disabled_tools else 'no'}")
        
        tools_for_display = director.get_tools_for_display(show_disabled=args.show_disabled_tools)
        for tool_id, tool in tools_for_display.items():
            enabled = director.is_tool_enabled(tool_id)
        ...
        print('  python ai_director.py --profile base --request "thermal scan straits" --execute')
